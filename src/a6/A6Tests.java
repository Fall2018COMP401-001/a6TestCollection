...
public class A6Tests {
	...
	void constructor() {
		...
		assertTrue(test.getBottom() == 5);
		
	}
	
	@Test
	void RegionImplIntersect() {
		try {
			test.intersect(null);
		...
		try {
			test.intersect(new RegionImpl (0, 5, 7, 0));
		} catch (NoIntersectionException c) {
			assertTrue(true);
		}
	...
	@Test
	void RegionImplUnionIntersect() {
		assertEquals(test.union(null), test);
		assertEquals(test.union(new RegionImpl(2,3,4,5)), new RegionImpl(1,2,4,5));
	}
	
	@Test
	void ObservablePictureConstructor() {
		try {
		...
		assertEquals(test3.getWidth(), 3 );
		assertEquals(test3.getHeight(), 2);
